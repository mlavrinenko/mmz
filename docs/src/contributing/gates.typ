...
Specifically, a gate's scopes should include the toolchain pins — both
`rust-toolchain.toml` and `flake.lock`, since the dev shell is what actually
selects the compiler. A toolchain bump that did not bust the clippy cache would
mean shipping against lints nobody ran.
