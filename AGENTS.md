...
- `mmz` dogfoods itself: [mmz.yaml](mmz.yaml) declares the rules and `just check`
  wraps `cargo test`/`clippy`/`fmt`/`machete` with the locally built binary
  (`_mmz` recipe builds it first), so a no-op `just check` skips them. When you
  add or rename a check command in the recipe, update mmz.yaml's rules to match.
